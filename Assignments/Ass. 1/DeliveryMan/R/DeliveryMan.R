...
#' 
#' @param carReady Your function that takes three arguments:
#' 
#' (1) a list of two matrices giving the traffice conditions. The first matrix is named 'hroads' and gives a matrix of traffice conditions on the horizontal roads. The second matrix is named 'vroads' and gives a matrix of traffic conditional on the vertical roads.
#' 
...
runDeliveryMan <- function (carReady=manualDM,dim=10,turns=2000,
  ...
  roads=makeRoadMatrices(dim)                             # Make road grid
  car=list(x=1,y=1,wait=0,load=0,nextMove=NA,mem=list())  # Create car
  packages=matrix(sample(1:dim,replace=T,5*del),ncol=5)   # Create matrix packages with some values
  packages[,5]=rep(0,del)                                 # Set all packages statuses as 'not picked up'
  
  # Game runs to turn 2000, if all packages has been delivered before turn 2000 you win, otherwise you fail
  for (i in 1:turns) {
    ...
    # If the car is not supposed to wait at a road7intersection/whatever
    if (car$wait==0) {
      
        # If car holds no packages
        if (car$load==0) {
        
            # on = the package the car sits on, 0 if car sits on no package
            on=packageOn(car$x,car$y,packages)
            
            # If car is on a package, pick it up
            if (on!=0) {
              packages[on,5]=1 # Set [on,5] to 1
              car$load=on
            }
        
        # If we are on the held packages destination
        } else if (packages[car$load,3]==car$x && packages[car$load,4]==car$y) {
            
            # Set current package as delivered
            packages[car$load,5]=2
            car$load=0
            
            # Check if we've delived all packages
...
updateRoads<-function(hroads,vroads) {
    ...
    # Generate a set of random numbers between 1 and 0
    r1=runif(length(hroads))
    r2=runif(length(hroads))
    
    # For each road in the raod map
    for (i in 1:length(hroads)) {
        
        # There is a 5% chance of increasing the cost of a road, and
        # there is a 5% chance of decreasing the cost of a road.
        # 
        # Min cost of a road is 1.
